...
export default function TodoManager() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('WARNING: Deleting a user todo item will permanently destroy it. Proceed?')) return;
    try {
  ...
  if (loading) return <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center' }}>Loading Todos...</div>;
  ...
  return (
    <>
      <header style={{ marginBottom: '2.5rem' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: 600 }}>Global Todo Lists</h1>
        <p style={{ color: 'var(--text-muted)' }}>Audit and manage all user scratchpad objectives.</p>
      </header>

      <div className="glass-panel" style={{ padding: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: 500 }}>Global Todos ({data.length})</h3>
                ...
                <tr key={t.id} style={{ borderBottom: '1px solid var(--bg-surface)' }} className="animate-fade-in">
                  ...
                  <td style={{ padding: '1rem' }}>
                    <span style={{ 
                      ...
                      background: t.is_completed ? 'rgba(16, 185, 129, 0.1)' : 'rgba(255, 255, 255, 0.1)',
                      color: t.is_completed ? 'var(--success)' : 'var(--text-muted)'
                    }}>
                      {t.is_completed ? <CheckCircle size={14} /> : <Circle size={14} />} {t.is_completed ? 'Done' : 'Active'}
                    </span>
                  </td>
                  <td style={{ padding: '1rem', fontWeight: 500 }}>{t.title}</td>
                  <td style={{ padding: '1rem', color: 'var(--text-muted)' }}>{t.user_email}</td>
                  <td style={{ padding: '1rem', textAlign: 'right' }}>
                    <button onClick={() => handleDelete(t.id)} className="btn glass-panel" style={{ padding: '0.5rem', border: '1px solid rgba(239, 68, 68, 0.3)' }} title="Delete Todo">
